      ...
      A: Address,
{
  ...
  /// Perform exact match lookup of `ip`/`masklen` and return the
  /// value and all the matching supernets.
  ///
  /// # Examples
  ///
  /// ```
  ...
  /// let exact = Ipv6Addr::new(0x2001, 0xdb8, 0xdead, 0xbeef, 0xcafe, 0xbabe, 0, 1);
  /// table.insert(less_specific, 32, "foo");
  /// table.insert(more_specific, 48, "bar");
  /// table.insert(most_specific, 64, "baz");
  /// table.insert(exact, 128, "quux");
  ///
  /// let result = table.supernets_exact(exact, 128);
  /// assert_eq!( result, vec![&"foo", &"bar", &"baz", &"quux"]);
  ///
  /// let lookupip = Ipv6Addr::new(0x2001, 0xdb8, 0xcafe, 0xf00, 0xf00, 0xf00, 0, 1);
  /// let result = table.supernets_exact(lookupip, 128);
  /// assert_eq!(result, Vec::<&&str>::new());
  /// ```
  pub fn supernets_exact(&self, ip: A, masklen: u32) -> Vec<&T> {
    self.inner.exact_match_supernets(&ip.nibbles().as_ref(), masklen)
  }

  /// Perform a longest match lookup of `ip` and return all the matching
  /// supernets for the prefix, designated by ip, masklen along with its value.
  ///
  /// # Example
  ///
  /// ```
  ...
  ///                              0xcafe, 0xbabe, 0, 1);
  /// let result = table.supernets(lookupip, 128);
  /// assert_eq!(result, vec![(less_specific, 32, &"foo"), (more_specific, 48, &"bar"), (most_specific, 64, &"baz")]);
  ///
  /// let lookupip = Ipv6Addr::new(0x2001, 0xdb8, 0xcafe, 0xf00,
  ///                              0xf00, 0xf00, 0, 1);
  /// let result = table.supernets(lookupip, 128);
  /// assert_eq!(result, vec![(less_specific, 32, &"foo")]);
  /// ```
  pub fn supernets(&self, ip: A, masklen: u32) -> Vec<(A, u32, &T)> {
    self.inner.longest_match_supernets(&ip.nibbles().as_ref()).iter()
