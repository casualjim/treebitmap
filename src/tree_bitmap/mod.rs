...
impl<T: Sized> TreeBitmap<T> {
  ...
  pub fn longest_match_supernets(&self, nibbles: &[u8]) -> Vec<(u32, &T)> {
    let mut cur_hdl = self.root_handle();
    let mut cur_index = 0;
    let mut bits_matched = 0;
    let mut bits_searched = 0;
  ...
  pub fn exact_match_supernets(&self, nibbles: &[u8], masklen: u32) -> Vec<&T> {
    let guesses = self.longest_match_supernets(nibbles);
    if !guesses.iter().any(|(mask, _)| *mask == masklen) {
      return vec![]
    }
    guesses.iter().map(|v|v.1).collect()
  }
